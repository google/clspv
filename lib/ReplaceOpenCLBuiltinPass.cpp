...
struct ReplaceOpenCLBuiltinPass final : public ModulePass {
  ...
  bool replaceExp10(Function &F, const std::string& basename, int vsize, int blen);
  bool replaceLog10(Function &F, const std::string& basename, int vsize, int blen);
  ...
  bool replaceIsInfAndIsNan(Function &F, spv::Op Op, int32_t isvec);
  bool replaceIsFinite(Function &F);
  bool replaceAllAndAny(Function &F, spv::Op Op);
  ...
  bool replaceStep(Function &F, bool is_smooth, int vsize);
  ...
  bool replaceFract(Function &F, int vsize);
...
bool ReplaceOpenCLBuiltinPass::runOnFunction(Function &F) {
  ...
  switch (FI.getType()) {
  case Builtins::EAbs:
    if (!FI.getParameter(0).sign) {
      return replaceCallsWithValue(F, [](CallInst *CI) { return CI->getOperand(0); });
    }
    break;
  case Builtins::EAbsDiff:
    return replaceAbsDiff(F, FI.getParameter(0).sign);
  case Builtins::ECopysign:
    return replaceCopysign(F);

  case Builtins::EHalfRecip:
  case Builtins::ENativeRecip:
  ...
  case Builtins::EHalfDivide:
  case Builtins::ENativeDivide:
    return replaceCallsWithValue(F, [](CallInst *CI) {
  ...
  case Builtins::EExp10:
  case Builtins::EHalfExp10:
  case Builtins::ENativeExp10: {
    auto& P0 = FI.getParameter(0);
    return replaceExp10(F, FI.getName(), P0.vsiz, P0.blen);
  }

  case Builtins::ELog10:
  case Builtins::EHalfLog10:
  case Builtins::ENativeLog10: {
    auto& P0 = FI.getParameter(0);
    return replaceLog10(F, FI.getName(), P0.vsiz, P0.blen);
  }
  ...
  case Builtins::EBarrier:
  case Builtins::EWorkGroupBarrier:
    return replaceBarrier(F);

  case Builtins::EMemFence:
    return replaceMemFence(F, spv::MemorySemanticsSequentiallyConsistentMask);
  case Builtins::EReadMemFence:
    return replaceMemFence(F, spv::MemorySemanticsAcquireMask);
  case Builtins::EWriteMemFence:
    return replaceMemFence(F, spv::MemorySemanticsReleaseMask);

    // Relational
  case Builtins::EIsequal:
    return replaceRelational(F, CmpInst::FCMP_OEQ, FI.getParameter(0).vsiz ? -1 : 1);
  case Builtins::EIsgreater:
    return replaceRelational(F, CmpInst::FCMP_OGT, FI.getParameter(0).vsiz ? -1 : 1);
  case Builtins::EIsgreaterequal:
    return replaceRelational(F, CmpInst::FCMP_OGE, FI.getParameter(0).vsiz ? -1 : 1);
  case Builtins::EIsless:
    return replaceRelational(F, CmpInst::FCMP_OLT, FI.getParameter(0).vsiz ? -1 : 1);
  case Builtins::EIslessequal:
    return replaceRelational(F, CmpInst::FCMP_OLE, FI.getParameter(0).vsiz ? -1 : 1);
  case Builtins::EIsnotequal:
    return replaceRelational(F, CmpInst::FCMP_ONE, FI.getParameter(0).vsiz ? -1 : 1);


  case Builtins::EIsinf: flag = true;
  case Builtins::EIsnan: {
    bool isvec = FI.getParameter(0).vsiz != 0;
    return replaceIsInfAndIsNan(F, flag ? spv::OpIsInf : spv::OpIsNan, isvec ? -1 : 1);
  }

  case Builtins::EIsfinite:
    return replaceIsFinite(F);

  case Builtins::EAll: flag = true;
  case Builtins::EAny: {
    bool isvec = FI.getParameter(0).vsiz != 0;
    return replaceAllAndAny(F, !isvec ? spv::OpNop : (flag ? spv::OpAll : spv::OpAny));
  }

  case Builtins::EUpsample:
    return replaceUpsample(F);

  case Builtins::ERotate:
    return replaceRotate(F);

  case Builtins::EConvert:
    return replaceConvert(F, FI.getParameter(0).sign, FI.getReturnType().sign);

  case Builtins::EAtomicInc:
    return replaceAtomics(F, spv::OpAtomicIIncrement);
  case Builtins::EAtomicDec:
    return replaceAtomics(F, spv::OpAtomicIDecrement);
  case Builtins::EAtomicCmpxchg:
    return replaceAtomics(F, spv::OpAtomicCompareExchange);
  case Builtins::EAtomicAdd:
    return replaceAtomics(F, llvm::AtomicRMWInst::Add);
  case Builtins::EAtomicSub:
    return replaceAtomics(F, llvm::AtomicRMWInst::Sub);
  case Builtins::EAtomicXchg:
    return replaceAtomics(F, llvm::AtomicRMWInst::Xchg);
  case Builtins::EAtomicMin:
    return replaceAtomics(F, FI.getParameter(0).sign ? llvm::AtomicRMWInst::Min : llvm::AtomicRMWInst::UMin);
  case Builtins::EAtomicMax:
    return replaceAtomics(F, FI.getParameter(0).sign ? llvm::AtomicRMWInst::Max : llvm::AtomicRMWInst::UMax);
  case Builtins::EAtomicAnd:
    return replaceAtomics(F, llvm::AtomicRMWInst::And);
  case Builtins::EAtomicOr:
    return replaceAtomics(F, llvm::AtomicRMWInst::Or);
  case Builtins::EAtomicXor:
    return replaceAtomics(F, llvm::AtomicRMWInst::Xor);

  case Builtins::ECross:
    if (FI.getParameter(0).vsiz == 4) {
      return replaceCross(F);
    }
    break;

  case Builtins::EFract:
    if (FI.getParameterCount()) {
      return replaceFract(F, FI.getParameter(0).vsiz);
    }
    break;

  case Builtins::EMadHi: flag = true;
  case Builtins::EMulHi:
    return replaceMulHi(F, FI.getParameter(0).sign, flag);

  case Builtins::EMad:
  case Builtins::EMad24: flag = true;
  case Builtins::EMul24:
    return replaceMul(F, FI.getParameter(0).type == llvm::Type::FloatTyID, flag);

  case Builtins::ESelect:
    return replaceSelect(F);

  case Builtins::EBitselect:
    return replaceBitSelect(F);

  case Builtins::EVload:
    return replaceVload(F);

  case Builtins::EVloadaHalf: {
    auto& name = FI.getName();
    if (!name.compare(0, 8, "__clspv_")) {
      int vsize = FI.getParameter(0).vsiz;
      if (!vsize) {
        vsize = std::atoi(&name[name.size()-1]);
      }
      switch (vsize) {
      case 2:                            return replaceClspvVloadaHalf2(F);
      case 4:                            return replaceClspvVloadaHalf4(F);
      default:                           assert(0); break;
      }
    }
    // break; // fall-thru for non-clspv
  }
  case Builtins::EVloadHalf: { 
    auto& name = FI.getName();
    int vsize = FI.getParameter(0).vsiz;
    if (!vsize) {
      vsize = std::atoi(&name[name.size()-1]);
    ...
    switch (vsize) {
    case 0:                            return replaceVloadHalf(F);
    case 2:                            return replaceVloadHalf2(F);
    case 4:                            return replaceVloadHalf4(F);
    default:                           assert(0); break;
    }
    break;
  }

  case Builtins::EVstore:
    return replaceVstore(F);
      
  case Builtins::EVstoreHalf:
  case Builtins::EVstoreaHalf:
    switch (FI.getParameter(0).vsiz) {
    case 0:                            return replaceVstoreHalf(F);
    case 2:                            return replaceVstoreHalf2(F);
    case 4:                            return replaceVstoreHalf4(F);
    default:                           assert(0); break;
    }
    break;

  case Builtins::ESmoothstep: flag = true;
  case Builtins::EStep: {
    int vsize = FI.getParameter(FI.getParameterCount()-1).vsiz;
    if (FI.getParameter(0).vsiz == 0 && vsize != 0) {
      return replaceStep(F, flag, vsize);
    }
    break;
  }

  case Builtins::ESignbit:
    return replaceSignbit(F, FI.getParameter(0).vsiz != 0);

  case Builtins::EReadImageh:
    return replaceHalfReadImage(F);
  case Builtins::EReadImagef:
  case Builtins::EReadImagei:
  case Builtins::EReadImageui: {
    if (FI.getParameter(1).name == "ocl_sampler") {
      if (FI.getParameter(2).type == llvm::Type::IntegerTyID) {
  ...
  case Builtins::EWriteImageh:
    return replaceHalfWriteImage(F);

  case Builtins::EGetGlobalId:
  case Builtins::EGetGlobalOffset:
  case Builtins::EGetGlobalSize:
  case Builtins::EGetGroupId:
  case Builtins::EGetLocalId:
  case Builtins::EGetLocalSize:
  case Builtins::EGetNumGroups:
  case Builtins::EGetWorkDim:
  case Builtins::EBuiltinNone:                  break;
...
bool ReplaceOpenCLBuiltinPass::replaceExp10(Function &F, const std::string& basename, int vsize, int blen) {
  // convert to natural
  auto slen = basename.length() - 2;
  std::string NewFName = "_Z" + std::to_string(slen) + basename.substr(0, slen);
  if (vsize) {
    NewFName += "Dv" + std::to_string(vsize) + "_";
  }
  switch (blen) {
  case 4:  NewFName += "f"; break;
  case 8:  NewFName += "d"; break;
  default: assert(0); break;
  }
...
bool ReplaceOpenCLBuiltinPass::replaceLog10(Function &F, const std::string& basename, int vsize, int blen) {
  // convert to natural
  auto slen = basename.length() - 2;
  std::string NewFName = "_Z" + std::to_string(slen) + basename.substr(0, slen);
  if (vsize) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      return BinaryOperator::Create(Instruction::FMul, ConstantFP::get(Arg->getType(), Ln10),
                                    NewCI, "", CI);
...
bool ReplaceOpenCLBuiltinPass::replaceBarrier(Function &F) {
  ...
  return replaceCallsWithValue(F, [](CallInst *CI) {
      ...
      const auto ConstantSequentiallyConsistent = ConstantInt::get(Arg->getType(),
      ...
      const auto MemorySemanticsWorkgroup = BinaryOperator::Create(Instruction::Shl,
                                                                   LocalMemFenceMask,
      ...
      const auto MemorySemanticsUniform = BinaryOperator::Create(Instruction::Shl,
                                                                 GlobalMemFenceMask,
...
bool ReplaceOpenCLBuiltinPass::replaceMemFence(Function &F, uint32_t semantics) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      // Map CLK_LOCAL_MEM_FENCE to MemorySemanticsWorkgroupMemoryMask.
      const auto LocalMemFenceMask = BinaryOperator::Create(Instruction::And, LocalMemFence,
                                                            Arg, "", CI);
      const auto WorkgroupShiftAmount =
      ...
      // Map CLK_GLOBAL_MEM_FENCE to MemorySemanticsUniformMemoryMask.
      const auto GlobalMemFenceMask = BinaryOperator::Create(Instruction::And, GlobalMemFence, Arg, "", CI);
      const auto UniformShiftAmount =
      ...
      MemorySemantics = BinaryOperator::Create(Instruction::Or, MemorySemantics, MemorySemanticsUniform, "", CI);
...
bool ReplaceOpenCLBuiltinPass::replaceRelational(Function &F, CmpInst::Predicate P, int32_t C) {
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      const auto Cmp = CmpInst::Create(Instruction::FCmp, Predicate, Arg1, Arg2, "", CI);

      return SelectInst::Create(Cmp, TrueValue, FalseValue, "", CI);
    });
}

bool ReplaceOpenCLBuiltinPass::replaceIsInfAndIsNan(Function &F, spv::Op Op, int32_t C) {
  Module& M = *F.getParent();
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      auto& C = M.getContext();
      Type* CorrespondingBoolTy = Type::getInt1Ty(C);
      if (CITy->isVectorTy()) {
        CorrespondingBoolTy = VectorType::get(Type::getInt1Ty(C), CITy->getVectorNumElements());
      }
...
bool ReplaceOpenCLBuiltinPass::replaceAllAndAny(Function &F, spv::Op Op) {
  Module& M = *F.getParent();
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      auto Arg = CI->getOperand(0);
      ...
      if (Arg->getType() == Type::getInt32Ty(M.getContext())) {
        ...
        const auto Cmp = CmpInst::Create(Instruction::ICmp, CmpInst::ICMP_SLT, Arg, ZeroValue, "", CI);

        Value *SelectSource;

        // If we have a function to call, call it!
        const auto SPIRVOp = Op;

        if (SPIRVOp != spv::OpNop) {

          const auto BoolTy = Type::getInt1Ty(M.getContext());

          const auto NewCI = clspv::InsertSPIRVOp(CI, SPIRVOp, {Attribute::ReadNone}, BoolTy, {Cmp});
          SelectSource = NewCI;
...
bool ReplaceOpenCLBuiltinPass::replaceUpsample(Function &F) {
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      ...
      // Shift high operand
      auto ShiftAmount = ConstantInt::get(CI->getType(), HiType->getScalarSizeInBits());
      auto HiShifted = BinaryOperator::Create(Instruction::Shl, HiCast, ShiftAmount, "", CI);
...
bool ReplaceOpenCLBuiltinPass::replaceRotate(Function &F) {
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      ...
      auto ModMask = ConstantInt::get(SrcType, SrcType->getScalarSizeInBits() - 1);
      RotAmount = BinaryOperator::Create(Instruction::And, RotAmount, ModMask, "", CI);

      // Let's calc the amount by which to shift top bits down
      auto ScalarSize = ConstantInt::get(SrcType, SrcType->getScalarSizeInBits());
      auto DownAmount = BinaryOperator::Create(Instruction::Sub, ScalarSize, RotAmount, "", CI);

      // Now shift the bottom bits up and the top bits down
      auto LoRotated = BinaryOperator::Create(Instruction::Shl, SrcValue, RotAmount, "", CI);
      auto HiRotated = BinaryOperator::Create(Instruction::LShr, SrcValue, DownAmount, "", CI);

      // Finally OR the two shifted values
      return BinaryOperator::Create(Instruction::Or, LoRotated, HiRotated, "", CI);
    });
}

bool ReplaceOpenCLBuiltinPass::replaceConvert(Function &F, bool SrcIsSigned, bool DstIsSigned) {
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
...
bool ReplaceOpenCLBuiltinPass::replaceMulHi(Function &F, bool is_signed, bool is_mad) {
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      // Get arguments
...
bool ReplaceOpenCLBuiltinPass::replaceSelect(Function &F) {
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      ...
      auto Cmp = CmpInst::Create(Instruction::ICmp, Pred, PredicateValue, ZeroValue, "", CI);
...
bool ReplaceOpenCLBuiltinPass::replaceBitSelect(Function &F) {
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      ...
      if (OpType->getScalarType()->isFloatingPointTy()) {
        ...
        PredicateValue = CastInst::CreateZExtOrBitCast(PredicateValue, BitType, "", CI);
      ...
      auto NotPredicateValue = BinaryOperator::Create(Instruction::Xor, PredicateValue, AllOnes, "", CI);

      // Then put everything together
      auto BitsFalse = BinaryOperator::Create(Instruction::And, NotPredicateValue, FalseValue, "", CI);
      auto BitsTrue = BinaryOperator::Create(Instruction::And, PredicateValue, TrueValue, "", CI);

      Value *V = BinaryOperator::Create(Instruction::Or, BitsFalse, BitsTrue, "", CI);
...
bool ReplaceOpenCLBuiltinPass::replaceStep(Function &F, bool is_smooth, int vsize) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      ...
      if (is_smooth) {
        NewFName += std::to_string(10) + "smoothstepDv" + std::to_string(vsize) + "_fS_S_";
      } else {
        NewFName += std::to_string(4) + "stepDv" + std::to_string(vsize) + "_fS_";
      }
...
bool ReplaceOpenCLBuiltinPass::replaceSignbit(Function &F, bool is_vec) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      ...
      return BinaryOperator::Create(Op, Bitcast, ConstantInt::get(CI->getType(), 31), "", CI);
    });
}

bool ReplaceOpenCLBuiltinPass::replaceMul(Function &F, bool is_float, bool is_mad) {
  Module& M = *F.getParent();
  return replaceCallsWithValue(F, [&](CallInst *CI) -> llvm::Value* {
      ...
      auto I = BinaryOperator::Create(MulInst, CI->getArgOperand(0), CI->getArgOperand(1), "", CI);

      if (is_mad) {
        // The add instruction to use.
        auto AddInst = is_float ? Instruction::FAdd : Instruction::Add;

        I = BinaryOperator::Create(AddInst, I, CI->getArgOperand(2), "", CI);
      }
...
bool ReplaceOpenCLBuiltinPass::replaceVloadHalf(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      if (clspv::Option::F16BitStorage()) {
        ...
        auto ShortPointerTy = PointerType::get(ShortTy, Arg1->getType()->getPointerAddressSpace());
        ...
        auto IntPointerTy = PointerType::get(IntTy, Arg1->getType()->getPointerAddressSpace());
...
bool ReplaceOpenCLBuiltinPass::replaceVloadHalf2(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      auto IntTy = Type::getInt32Ty(M.getContext());
      auto Float2Ty = VectorType::get(Type::getFloatTy(M.getContext()), 2);
      auto NewPointerTy = PointerType::get(IntTy, Arg1->getType()->getPointerAddressSpace());
      auto NewFType = FunctionType::get(Float2Ty, IntTy, false);
...
bool ReplaceOpenCLBuiltinPass::replaceVloadHalf4(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      auto Float2Ty = VectorType::get(Type::getFloatTy(M.getContext()), 2);
      auto NewPointerTy = PointerType::get(Int2Ty, Arg1->getType()->getPointerAddressSpace());
      auto NewFType = FunctionType::get(Float2Ty, IntTy, false);
      ...
      auto X = ExtractElementInst::Create(Load, ConstantInt::get(IntTy, 0), "", CI);
      auto Y = ExtractElementInst::Create(Load, ConstantInt::get(IntTy, 1), "", CI);
      ...
      return new ShuffleVectorInst(Lo, Hi, ConstantVector::get(ShuffleMask), "", CI);
    });
}
...
bool ReplaceOpenCLBuiltinPass::replaceVstoreHalf(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      if (clspv::Option::F16BitStorage()) {
        auto ShortTy = Type::getInt16Ty(M.getContext());
        auto ShortPointerTy = PointerType::get(ShortTy, Arg2->getType()->getPointerAddressSpace());
        ...
        auto IntPointerTy = PointerType::get(IntTy, Arg2->getType()->getPointerAddressSpace());
...
bool ReplaceOpenCLBuiltinPass::replaceVstoreHalf2(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      auto Float2Ty = VectorType::get(Type::getFloatTy(F.getContext()), 2);
      auto NewPointerTy = PointerType::get(IntTy, Arg2->getType()->getPointerAddressSpace());
      auto NewFType = FunctionType::get(IntTy, Float2Ty, false);
...
bool ReplaceOpenCLBuiltinPass::replaceVstoreHalf4(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      auto Int2Ty = VectorType::get(IntTy, 2);
      auto Float2Ty = VectorType::get(Type::getFloatTy(F.getContext()), 2);
      auto NewPointerTy = PointerType::get(Int2Ty, Arg2->getType()->getPointerAddressSpace());
      auto NewFType = FunctionType::get(IntTy, Float2Ty, false);
      ...
      auto Combine = InsertElementInst::Create(UndefValue::get(Int2Ty), X, ConstantInt::get(IntTy, 0), "", CI);
      Combine = InsertElementInst::Create(Combine, Y, ConstantInt::get(IntTy, 1), "", CI);
...
bool ReplaceOpenCLBuiltinPass::replaceHalfReadImage(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      auto NewFType = FunctionType::get(VectorType::get(Type::getFloatTy(M.getContext()),
...
bool ReplaceOpenCLBuiltinPass::replaceHalfWriteImage(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      // Data
      types[2] = VectorType::get(Type::getFloatTy(M.getContext()),
                                 CI->getArgOperand(2)->getType()->getVectorNumElements());

      auto NewFType = FunctionType::get(Type::getVoidTy(M.getContext()), types, false);
...
bool ReplaceOpenCLBuiltinPass::replaceSampledReadImageWithIntCoords(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      auto NewFType = FunctionType::get(CI->getType(), {Arg0->getType(), Arg1->getType(), float_ty}, false);
...
bool ReplaceOpenCLBuiltinPass::replaceAtomics(Function &F, spv::Op Op) {
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      // We need to map the OpenCL constants to the SPIR-V equivalents.
      const auto ConstantScopeDevice = ConstantInt::get(IntTy, spv::ScopeDevice);
      const auto ConstantMemorySemantics = ConstantInt::get(IntTy, spv::MemorySemanticsUniformMemoryMask |
                                                            spv::MemorySemanticsSequentiallyConsistentMask);
...
bool ReplaceOpenCLBuiltinPass::replaceAtomics(Function &F, llvm::AtomicRMWInst::BinOp Op) {
  return replaceCallsWithValue(F, [&](CallInst *CI) {
...
bool ReplaceOpenCLBuiltinPass::replaceCross(Function &F) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      auto IntTy = Type::getInt32Ty(F.getContext());
      auto FloatTy = Type::getFloatTy(F.getContext());
      ...
      auto Arg0 = new ShuffleVectorInst(CI->getArgOperand(0), UndefValue::get(Vec4Ty),
                                        ConstantVector::get(DownShuffleMask), "", CI);
      auto Arg1 = new ShuffleVectorInst(CI->getArgOperand(1), UndefValue::get(Vec4Ty),
                                        ConstantVector::get(DownShuffleMask), "", CI);
...
bool ReplaceOpenCLBuiltinPass::replaceFract(Function &F, int vsize) {
  ...
  return replaceCallsWithValue(F, [&](CallInst *CI) {
      ...
      if (vsize) {
        floor_name += "Dv" + std::to_string(vsize) + "_f";
        fmin_name += "Dv" + std::to_string(vsize) + "_f";
        clspv_fract_name += "v" + std::to_string(vsize) + "f";
      } else {
        floor_name += "ff";
        fmin_name += "ff";
        clspv_fract_name += "f";
      ...
      if (!fmin_fn) {
        ...
        fmin_fn = cast<Function>(M.getOrInsertFunction(fmin_name, fn_ty).getCallee());
      ...
      if (!floor_fn) {
        ...
        floor_fn = cast<Function>(M.getOrInsertFunction(floor_name, fn_ty).getCallee());
      ...
      if (!clspv_fract_fn) {
        ...
        FunctionType *fn_ty = FunctionType::get(result_ty, {result_ty}, false);
        clspv_fract_fn = cast<Function>(M.getOrInsertFunction(clspv_fract_name, fn_ty).getCallee());
      ...
      const double kJustUnderOneScalar = ldexp(double((1 << num_significand_bits) - 1), -num_significand_bits);

      Constant *just_under_one = ConstantFP::get(result_ty->getScalarType(), kJustUnderOneScalar);
      if (result_ty->isVectorTy()) {
        just_under_one = ConstantVector::getSplat(result_ty->getVectorNumElements(), just_under_one);
      }
