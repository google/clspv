// RUN: clspv %target %s -o %t.spv -no-16bit-storage=ssbo -arch=spirv32
// RUN: spirv-dis -o %t2.spvasm %t.spv
// RUN: FileCheck %s < %t2.spvasm --check-prefixes=CHECK,CHECK-32
// RUN: spirv-val --target-env vulkan1.0 %t.spv

// RUN: clspv %target %s -o %t.spv -no-16bit-storage=ssbo -arch=spirv64
// RUN: spirv-dis -o %t2.spvasm %t.spv
...
// CHECK-32: %[[INDEX_INTO_UINT:[a-zA-Z0-9_]*]] = OpShiftRightLogical %[[UINT_TYPE_ID]] {{.*}} %[[CONSTANT_1_ID]]
...
// CHECK: %[[SHIFT_RAW:[a-zA-Z0-9_]*]] = OpShiftLeftLogical %[[UINT_TYPE_ID]] {{.*}} %[[CONSTANT_4_ID]]
