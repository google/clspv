...
define internal spir_func float @foo(ptr addrspace(1) %p) {
entry:
  %gep = getelementptr float, ptr addrspace(1) %p, i32 1
  %ld = load float, ptr addrspace(1) %gep
  ret float %ld
}

define internal spir_func float @bar(ptr addrspace(2) %p) {
entry:
  %gep = getelementptr float, ptr addrspace(2) %p, i32 2
  %ld = load float, ptr addrspace(2) %gep
  ret float %ld
}
