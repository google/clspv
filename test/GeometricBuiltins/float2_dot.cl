...
// CHECK: OpDecorate %[[FLOAT_ARG_STRUCT_TYPE_ID]] BufferBlock
// CHECK: OpDecorate %[[FLOAT2_DYNAMIC_ARRAY_TYPE_ID:[a-zA-Z0-9_]*]] ArrayStride 8
// CHECK: OpMemberDecorate %[[FLOAT2_ARG_STRUCT_TYPE_ID:[a-zA-Z0-9_]*]] 0 Offset 0
// CHECK: OpDecorate %[[FLOAT2_ARG_STRUCT_TYPE_ID]] BufferBlock
...
// CHECK: %[[FLOAT_UNIFORM_POINTER_TYPE_ID:[a-zA-Z0-9_]*]] = OpTypePointer StorageBuffer %[[FLOAT_TYPE_ID]]
...
// CHECK: %[[FLOAT2_UNIFORM_POINTER_TYPE_ID:[a-zA-Z0-9_]*]] = OpTypePointer StorageBuffer %[[FLOAT2_TYPE_ID]]
// CHECK: %[[FLOAT2_DYNAMIC_ARRAY_TYPE_ID]] = OpTypeRuntimeArray %[[FLOAT2_TYPE_ID]]
// CHECK: %[[FLOAT2_ARG_STRUCT_TYPE_ID]] = OpTypeStruct %[[FLOAT2_DYNAMIC_ARRAY_TYPE_ID]]
// CHECK: %[[FLOAT2_ARG_POINTER_TYPE_ID:[a-zA-Z0-9_]*]] = OpTypePointer StorageBuffer %[[FLOAT2_ARG_STRUCT_TYPE_ID]]
...
// CHECK: %[[ARG1_ID]] = OpVariable %[[FLOAT2_ARG_POINTER_TYPE_ID]] StorageBuffer
...
// CHECK: %[[B_ACCESS_CHAIN0_ID:[a-zA-Z0-9_]*]] = OpAccessChain %[[FLOAT2_UNIFORM_POINTER_TYPE_ID]] %[[ARG1_ID]] %[[CONSTANT_0_ID]] %[[CONSTANT_0_ID]]
// CHECK: %[[LOADB0_ID:[a-zA-Z0-9_]*]] = OpLoad %[[FLOAT2_TYPE_ID]] %[[B_ACCESS_CHAIN0_ID]]
// CHECK: %[[B_ACCESS_CHAIN1_ID:[a-zA-Z0-9_]*]] = OpAccessChain %[[FLOAT2_UNIFORM_POINTER_TYPE_ID]] %[[ARG1_ID]] %[[CONSTANT_0_ID]] %[[CONSTANT_1_ID]]
// CHECK: %[[LOADB1_ID:[a-zA-Z0-9_]*]] = OpLoad %[[FLOAT2_TYPE_ID]] %[[B_ACCESS_CHAIN1_ID]]
// CHECK: %[[OP_ID:[a-zA-Z0-9_]*]] = OpDot %[[FLOAT_TYPE_ID]] %[[LOADB0_ID]] %[[LOADB1_ID]]
// CHECK: OpStore %[[A_ACCESS_CHAIN_ID]] %[[OP_ID]]
// CHECK: OpReturn
// CHECK: OpFunctionEnd

void kernel __attribute__((reqd_work_group_size(1, 1, 1))) foo(global float* a, global float2* b)
{
  *a = dot(b[0], b[1]);
}
