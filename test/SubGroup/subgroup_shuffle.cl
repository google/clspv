...
// CHECK-DAG: [[shuffle_index:%[a-zA-Z0-9_]+]] = OpLoad [[uint]] {{.*}}Aligned 16
// CHECK-DAG: [[load_in:%[a-zA-Z0-9_]+]] = OpLoad [[uint]] {{.*}}Aligned 4
// CHECK-DAG: [[shuffled:%[a-zA-Z0-9_]+]] = OpGroupNonUniformShuffle [[uint]] [[uint_3]] [[load_in]] [[shuffle_index]]
// CHECK-DAG: OpStore {{.*}} [[shuffled]]

#pragma OPENCL EXTENSION cl_khr_subgroups : enable

kernel void test(global int* out, global int* in, int shuffle_index) {
  int gid = get_global_id(0);
  int value = in[gid];
  int shuffled_value = sub_group_shuffle(value, shuffle_index);
