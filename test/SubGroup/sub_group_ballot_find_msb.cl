...
// CHECK-DAG: [[msb:%[a-zA-Z0-9_]+]] = OpGroupNonUniformBallotFindMSB [[uint]] [[subgroup_scope]] [[mask]]
// CHECK-DAG: [[ins:%[a-zA-Z0-9_]+]] = OpCompositeInsert [[vec4]] [[msb]] {{%[a-zA-Z0-9_]+}} 0
...
kernel void test(global uint4* out, global uint4* in) {
  ...
  uint4 mask = in[gid];
  out[gid] = sub_group_ballot_find_msb(mask);
}
