...
; CHECK-DAG: [[func_ty:%[a-zA-Z0-9_]+]] = OpTypeFunction %{{.*}} [[ptr]]

; CHECK: OpFunction %{{.*}} None [[func_ty]]
; CHECK: [[param:%[a-zA-Z0-9_]+]] = OpFunctionParameter [[ptr]]
; CHECK: [[gep:%[a-zA-Z0-9_]+]] = OpUntypedAccessChainKHR [[ptr]] [[rta]] [[param]] [[uint_0]]
...
define void @foo(ptr addrspace(1) %data) {
entry:
  %0 = getelementptr [0 x i32], ptr addrspace(1) %data, i32 0, i32 0
...
entry:
  ...
  call void @foo(ptr addrspace(1) %0)
  ret void
}
