...
; The data interpretation reuses the SSBO block type since it isn't differentiated
; from the wg variable type.
; CHECK: [[gep:%[a-zA-Z0-9_]+]] = OpUntypedAccessChainKHR [[ptr_wg]] [[ssbo_block]] [[wg_var]] [[uint_0]] [[uint_0]]
; CHECK: OpStore [[gep]]
; CHECK: OpControlBarrier
; CHECK: OpStore [[test_wg2]]
; CHECK: OpControlBarrier
; CHECK: OpLoad [[uint]] [[test_wg2]]
...
entry:
  ...
  store i32 %add.i.i, ptr addrspace(3) %1, align 4, !alias.scope !25
  ...
  store i32 %10, ptr addrspace(1) %5, align 4, !noalias !25
  ret void
}
