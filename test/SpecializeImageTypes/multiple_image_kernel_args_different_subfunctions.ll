...
; CHECK: call spir_func <4 x float> @foo(ptr addrspace(1) %ro,
; CHECK: call spir_func void @bar(ptr addrspace(1) %wo,
; CHECK-DAG: define spir_func <4 x float> @foo
; CHECK-DAG: call spir_func <4 x float> @_Z11read_imagef28[[RO_IM:ocl_image2d_ro.float.sampled]]11ocl_samplerDv2_f(ptr addrspace(1) %ro
; CHECK-DAG: define spir_func void @bar
; CHECK-DAG: call spir_func void @_Z12write_imagei18[[WO_IM:ocl_image2d_wo.int]]Dv2_iDv4_i(ptr addrspace(1) %wo
...
define spir_kernel void @k1(ptr addrspace(1) %ro, ptr addrspace(1) %wo, <2 x float> %coord) local_unnamed_addr {
entry:
  %call = tail call spir_func <4 x float> @foo(ptr addrspace(1) %ro, <2 x float> %coord)
  tail call spir_func void @bar(ptr addrspace(1) %wo, <2 x float> %coord, <4 x float> %call)
  ret void
}

define spir_func <4 x float> @foo(ptr addrspace(1) %ro, <2 x float> %coord) {
entry:
...
define spir_func void @bar(ptr addrspace(1) %wo, <2 x float> %coord, <4 x float> %data) {
entry:
