...
; CHECK: call spir_func void @foo(ptr addrspace(1) %ro, ptr addrspace(1) %wo
...
entry:
  tail call spir_func void @foo(ptr addrspace(1) %ro, ptr addrspace(1) %wo, <2 x float> %coord)
  ret void
}

define spir_func void @foo(ptr addrspace(1) %ro, ptr addrspace(1) %wo, <2 x float> %coord) {
entry:
