...
define spir_kernel void @read_float1(%opencl.image2d_ro_t addrspace(1)* %image, <2 x float> %coord, <4 x float> addrspace(1)* nocapture %data) local_unnamed_addr {
entry:
...
define spir_kernel void @read_float2(%opencl.image2d_ro_t addrspace(1)* %image, <2 x float> %coord, <4 x float> addrspace(1)* nocapture %data) local_unnamed_addr {
entry:
  %call = tail call spir_func <4 x float> @bar(%opencl.image2d_ro_t addrspace(1)* %image, <2 x float> %coord)
  store <4 x float> %call, <4 x float> addrspace(1)* %data, align 16
  ret void
