...
define spir_kernel void @write_uint(%opencl.image2d_array_wo_t addrspace(1)* %image, <4 x i32> %coord, <4 x i32> addrspace(1)* nocapture %data) local_unnamed_addr #0 {
entry:
  %ld = load <4 x i32>, <4 x i32> addrspace(1)* %data, align 16
  call spir_func void @_Z13write_imageui20ocl_image2d_array_woDv4_iDv4_j(%opencl.image2d_array_wo_t addrspace(1)* %image, <4 x i32> %coord, <4 x i32> %ld) #2
  ret void
}

declare spir_func void @_Z13write_imageui20ocl_image2d_array_woDv4_iDv4_j(%opencl.image2d_array_wo_t addrspace(1)*, <4 x i32>, <4 x i32>) local_unnamed_addr #1
