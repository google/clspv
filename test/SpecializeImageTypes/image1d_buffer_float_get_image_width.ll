...
define spir_kernel void @write_float(%opencl.image1d_buffer_wo_t addrspace(1)* %image, i32 %coord, <4 x float> addrspace(1)* nocapture %data) local_unnamed_addr #0 {
entry:
  %ld = load <4 x float>, <4 x float> addrspace(1)* %data, align 16
  call spir_func void @_Z12write_imagef21ocl_image1d_buffer_woiDv4_f(%opencl.image1d_buffer_wo_t addrspace(1)* %image, i32 %coord, <4 x float> %ld) #2
  %h = tail call spir_func i32 @_Z15get_image_width21ocl_image1d_buffer_wo(%opencl.image1d_buffer_wo_t addrspace(1)* %image)
  ret void
}

declare spir_func void @_Z12write_imagef21ocl_image1d_buffer_woiDv4_f(%opencl.image1d_buffer_wo_t addrspace(1)*, i32, <4 x float>) local_unnamed_addr #1

declare spir_func i32 @_Z15get_image_width21ocl_image1d_buffer_wo(%opencl.image1d_buffer_wo_t addrspace(1)*) #2
