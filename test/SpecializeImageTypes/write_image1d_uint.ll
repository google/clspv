...
define spir_kernel void @write_uint(%opencl.image1d_wo_t addrspace(1)* %image, i32 %coord, <4 x i32> addrspace(1)* nocapture %data) local_unnamed_addr #0 {
entry:
  %ld = load <4 x i32>, <4 x i32> addrspace(1)* %data, align 16
  call spir_func void @_Z13write_imageui14ocl_image1d_woiDv4_j(%opencl.image1d_wo_t addrspace(1)* %image, i32 %coord, <4 x i32> %ld) #2
  ret void
}

declare spir_func void @_Z13write_imageui14ocl_image1d_woiDv4_j(%opencl.image1d_wo_t addrspace(1)*, i32, <4 x i32>) local_unnamed_addr #1
