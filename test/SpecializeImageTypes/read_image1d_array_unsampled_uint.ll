...
define spir_kernel void @read_uint(%opencl.image1d_array_ro_t addrspace(1)* %image, <2 x i32> %coord, <4 x i32> addrspace(1)* nocapture %data) local_unnamed_addr #0 {
entry:
  %call = tail call spir_func <4 x i32> @_Z12read_imageui20ocl_image1d_array_roDv2_i(%opencl.image1d_array_ro_t addrspace(1)* %image, <2 x i32> %coord) #3
  store <4 x i32> %call, <4 x i32> addrspace(1)* %data, align 16
  ret void
}

declare spir_func <4 x i32> @_Z12read_imageui20ocl_image1d_array_roDv2_i(%opencl.image1d_array_ro_t addrspace(1)*, <2 x i32>) local_unnamed_addr #1
