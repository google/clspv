...
define spir_kernel void @test(<2 x i32> %srcA, <2 x i32> %srcB, ptr addrspace(1) noundef align 32 %dst, <8 x i32> %mask) {
entry:
...
; CHECK: [[srcA0:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask0mod]]
...
; CHECK: [[srcA1:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask1mod]]
; CHECK: [[srcB1:%[^ ]+]] = extractelement <2 x i32> %srcB, i32 [[mask1mod]]
; CHECK: [[mask1mod2:%[^ ]+]] = urem i32 [[mask1]], 4
; CHECK: [[cmp1:%[^ ]+]] = icmp sge i32 [[mask1mod2]], 2
...
; CHECK: [[srcA2:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask2mod]]
; CHECK: [[srcB2:%[^ ]+]] = extractelement <2 x i32> %srcB, i32 [[mask2mod]]
; CHECK: [[mask2mod2:%[^ ]+]] = urem i32 [[mask2]], 4
; CHECK: [[cmp2:%[^ ]+]] = icmp sge i32 [[mask2mod2]], 2
...
; CHECK: [[srcA3:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask3mod]]
; CHECK: [[srcB3:%[^ ]+]] = extractelement <2 x i32> %srcB, i32 [[mask3mod]]
; CHECK: [[mask3mod2:%[^ ]+]] = urem i32 [[mask3]], 4
; CHECK: [[cmp3:%[^ ]+]] = icmp sge i32 [[mask3mod2]], 2
...
; CHECK: [[srcA4:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask4mod]]
; CHECK: [[srcB4:%[^ ]+]] = extractelement <2 x i32> %srcB, i32 [[mask4mod]]
; CHECK: [[mask4mod2:%[^ ]+]] = urem i32 [[mask4]], 4
; CHECK: [[cmp4:%[^ ]+]] = icmp sge i32 [[mask4mod2]], 2
...
; CHECK: [[srcA5:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask5mod]]
; CHECK: [[srcB5:%[^ ]+]] = extractelement <2 x i32> %srcB, i32 [[mask5mod]]
; CHECK: [[mask5mod2:%[^ ]+]] = urem i32 [[mask5]], 4
; CHECK: [[cmp5:%[^ ]+]] = icmp sge i32 [[mask5mod2]], 2
...
; CHECK: [[srcA6:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask6mod]]
; CHECK: [[srcB6:%[^ ]+]] = extractelement <2 x i32> %srcB, i32 [[mask6mod]]
; CHECK: [[mask6mod2:%[^ ]+]] = urem i32 [[mask6]], 4
; CHECK: [[cmp6:%[^ ]+]] = icmp sge i32 [[mask6mod2]], 2
...
; CHECK: [[srcA7:%[^ ]+]] = extractelement <2 x i32> %srcA, i32 [[mask7mod]]
; CHECK: [[srcB7:%[^ ]+]] = extractelement <2 x i32> %srcB, i32 [[mask7mod]]
; CHECK: [[mask7mod2:%[^ ]+]] = urem i32 [[mask7]], 4
; CHECK: [[cmp7:%[^ ]+]] = icmp sge i32 [[mask7mod2]], 2
