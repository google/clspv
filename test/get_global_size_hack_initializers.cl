...
// CHECK: OpEntryPoint GLCompute %[[FOO_ID:[a-zA-Z0-9_]*]] "foo" %[[NUM_WORKGROUPS_VAR_ID:[a-zA-Z0-9_]*]]
...
// CHECK: OpDecorate %[[WORKGROUP_SIZE_ID:[a-zA-Z0-9_]*]] BuiltIn WorkgroupSize
// CHECK: OpDecorate %[[NUM_WORKGROUPS_VAR_ID]] BuiltIn NumWorkgroups
...
// CHECK: %[[GET_GLOBAL_SIZE_TYPE_ID:[a-zA-Z0-9_]*]] = OpTypeFunction %[[UINT_TYPE_ID]] %[[UINT_TYPE_ID]]
...
// CHECK: %[[WORKGROUP_SIZE_ID]] = OpConstantComposite %[[UINT3_TYPE_ID]] %[[CONSTANT_1_ID]] %[[CONSTANT_1_ID]] %[[CONSTANT_1_ID]]
// CHECK: %[[WORKGROUP_SIZE_VAR_ID:[a-zA-Z0-9_]*]] = OpVariable %[[UINT3_PRIVATE_POINTER_TYPE_ID]] Private %[[WORKGROUP_SIZE_ID]]
// CHECK: %[[NUM_WORKGROUPS_VAR_ID]] = OpVariable %[[UINT3_INPUT_POINTER_TYPE_ID]] Input
...
// CHECK-NEXT: %[[ACCESS_CHAIN2_ID:[a-zA-Z0-9_]*]] = OpAccessChain %[[UINT_GLOBAL_POINTER_TYPE_ID]] %[[ARG0_ID]] %[[CONSTANT_0_ID]] %[[RESULT_ID]]
// CHECK-NEXT: OpStore %[[ACCESS_CHAIN2_ID]] %[[CONSTANT_1_ID]]
// CHECK-NEXT: OpReturn
...
// CHECK: %[[GET_GLOBAL_SIZE_ID]] = OpFunction %[[UINT_TYPE_ID]] Const %[[GET_GLOBAL_SIZE_TYPE_ID]]
...
// CHECK: %[[ACCESS_CHAIN3_ID:[a-zA-Z0-9_]*]] = OpAccessChain %[[UINT_PRIVATE_POINTER_TYPE_ID]] %[[WORKGROUP_SIZE_VAR_ID]] %[[SELECT1_ID]]
// CHECK: %[[LOAD2_ID:[a-zA-Z0-9_]*]] = OpLoad %[[UINT_TYPE_ID]] %[[ACCESS_CHAIN3_ID]]
// CHECK: %[[ACCESS_CHAIN4_ID:[a-zA-Z0-9_]*]] = OpAccessChain %[[UINT_INPUT_POINTER_TYPE_ID]] %[[NUM_WORKGROUPS_VAR_ID]] %[[SELECT1_ID]]
// CHECK: %[[LOAD3_ID:[a-zA-Z0-9_]*]] = OpLoad %[[UINT_TYPE_ID]] %[[ACCESS_CHAIN4_ID]]
// CHECK: %[[MUL_ID:[a-zA-Z0-9_]*]] = OpIMul %[[UINT_TYPE_ID]] %[[LOAD3_ID]] %[[LOAD2_ID]]
// CHECK: %[[SELECT2_ID:[a-zA-Z0-9_]*]] = OpSelect %[[UINT_TYPE_ID]] %[[COND_ID]] %[[MUL_ID]] %[[CONSTANT_1_ID]]
