...
// CHECK: %[[A_AND_B_TYPE_ID:[a-zA-Z0-9_]*]] = OpTypeFunction %[[THING_TYPE_ID]]
...
// CHECK: %[[CONSTANT_STRUCT0_ID:[a-zA-Z0-9_]*]] = OpConstantComposite %[[THING_TYPE_ID]] %[[CONSTANT_42_ID]] %[[CONSTANT_2_ID]]
// CHECK: %[[CONSTANT_13_ID:[a-zA-Z0-9_]*]] = OpConstant %[[FLOAT_TYPE_ID]] 13
// CHECK: %[[UNDEF_ID:[a-zA-Z0-9_]*]] = OpUndef %[[FLOAT_TYPE_ID]]
// CHECK: %[[CONSTANT_STRUCT1_ID:[a-zA-Z0-9_]*]] = OpConstantComposite %[[THING_TYPE_ID]] %[[CONSTANT_13_ID]] %[[UNDEF_ID]]
...
// CHECK: OpReturnValue %[[CONSTANT_STRUCT0_ID]]
// CHECK: OpFunctionEnd
...
// CHECK: %[[B_ID:[a-zA-Z0-9_]*]] = OpFunction %[[THING_TYPE_ID]] Const %[[A_AND_B_TYPE_ID]]
...
// CHECK: %[[INSERT_ID:[a-zA-Z0-9_]*]] = OpCompositeInsert %[[THING_TYPE_ID]] %[[Y_ID]] %[[CONSTANT_STRUCT1_ID]] 1
// CHECK: OpReturnValue %[[INSERT_ID]]
// CHECK: OpFunctionEnd

struct Thing b() {
  struct Thing thing = a();
  thing.x = 13.0f;
  return thing;
}
...
// CHECK: OpStore %[[ACCESS_CHAIN_ID]] %[[MUL_ID]]
// CHECK: OpReturn
// CHECK: OpFunctionEnd
