...
define dso_local spir_func %opencl.event_t* @foo(<16 x i8> addrspace(3)* %dst, <16 x i8> addrspace(1)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS3Dv16_cPU3AS1KS_jj9ocl_event(<16 x i8> addrspace(3)* %dst, <16 x i8> addrspace(1)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS3Dv16_cPU3AS1KS_jj9ocl_event(<16 x i8> addrspace(3)*, <16 x i8> addrspace(1)*, i32, i32, %opencl.event_t*)
...
; CHECK: [[ld:%[a-zA-Z0-9_.]+]] = load [16 x i8], [16 x i8] addrspace(1)* [[srci]]
; CHECK: store [16 x i8] [[ld]], [16 x i8] addrspace(3)* [[dsti]]
