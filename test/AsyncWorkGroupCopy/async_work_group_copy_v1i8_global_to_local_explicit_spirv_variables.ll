...
entry:
  %call = call spir_func ptr @_Z21async_work_group_copyPU3AS3cPU3AS1Kcj9ocl_event(ptr addrspace(3) %dst, ptr addrspace(1) %src, i32 %num_gentypes, ptr %event)
  ret ptr %call
}

declare spir_func ptr @_Z21async_work_group_copyPU3AS3cPU3AS1Kcj9ocl_event(ptr addrspace(3), ptr addrspace(1), i32, ptr)
...
; CHECK: [[loop]]:
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr i8, ptr addrspace(3) %dst, i32 [[phiiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr i8, ptr addrspace(1) %src, i32 [[phiiterator]]
; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]
; CHECK: [[ld:%[a-zA-Z0-9_.]+]] = load i8, ptr addrspace(1) [[srci]]
; CHECK: store i8 [[ld]], ptr addrspace(3) [[dsti]]
; CHECK: br label %[[cmp]]
