...
define dso_local spir_func %opencl.event_t* @foo(<3 x i8> addrspace(1)* %dst, <3 x i8> addrspace(3)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS1Dv3_cPU3AS3KS_jj9ocl_event(<3 x i8> addrspace(1)* %dst, <3 x i8> addrspace(3)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS1Dv3_cPU3AS3KS_jj9ocl_event(<3 x i8> addrspace(1)*, <3 x i8> addrspace(3)*, i32, i32, %opencl.event_t*)
...
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr <3 x i8>, <3 x i8> addrspace(1)* %dst, i32 [[dstiterator]]
