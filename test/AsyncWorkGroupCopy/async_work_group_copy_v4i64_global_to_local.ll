...
define dso_local spir_func %opencl.event_t* @foo(<4 x i64> addrspace(3)* %dst, <4 x i64> addrspace(1)* %src, i32 %num_gentypes, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS3Dv4_lPU3AS1KS_j9ocl_event(<4 x i64> addrspace(3)* %dst, <4 x i64> addrspace(1)* %src, i32 %num_gentypes, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS3Dv4_lPU3AS1KS_j9ocl_event(<4 x i64> addrspace(3)*, <4 x i64> addrspace(1)*, i32, %opencl.event_t*)
...
; CHECK: [[loop]]:
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr <4 x i64>, <4 x i64> addrspace(3)* %dst, i32 [[phiiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr <4 x i64>, <4 x i64> addrspace(1)* %src, i32 [[phiiterator]]
; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]
; CHECK: call void @_Z8spirv.op.63.PU3AS3Dv4_mPU3AS1Dv4_m(i32 63, <4 x i64> addrspace(3)* [[dsti]], <4 x i64> addrspace(1)* [[srci]])
; CHECK: br label %[[cmp]]
