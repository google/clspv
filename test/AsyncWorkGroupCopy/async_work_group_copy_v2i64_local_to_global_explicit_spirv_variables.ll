...
define dso_local spir_func %opencl.event_t* @foo(<2 x i64> addrspace(1)* %dst, <2 x i64> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv2_lPU3AS3KS_j9ocl_event(<2 x i64> addrspace(1)* %dst, <2 x i64> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv2_lPU3AS3KS_j9ocl_event(<2 x i64> addrspace(1)*, <2 x i64> addrspace(3)*, i32, %opencl.event_t*)
...
; CHECK: [[loop]]:
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr <2 x i64>, <2 x i64> addrspace(1)* %dst, i32 [[phiiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr <2 x i64>, <2 x i64> addrspace(3)* %src, i32 [[phiiterator]]
; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]
; CHECK: call void @_Z8spirv.op.63.PU3AS1Dv2_mPU3AS3Dv2_m(i32 63, <2 x i64> addrspace(1)* [[dsti]], <2 x i64> addrspace(3)* [[srci]])
; CHECK: br label %[[cmp]]
