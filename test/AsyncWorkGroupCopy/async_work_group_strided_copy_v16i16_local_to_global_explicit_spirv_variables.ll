...
define dso_local spir_func ptr @foo(ptr addrspace(1) %dst, ptr addrspace(3) %src, i32 %num_gentypes, i32 %stride, ptr %event) {
entry:
  %call = call spir_func ptr @_Z29async_work_group_strided_copyPU3AS1Dv16_sPU3AS3KS_jj9ocl_event(ptr addrspace(1) %dst, ptr addrspace(3) %src, i32 %num_gentypes, i32 %stride, ptr %event)
  ret ptr %call
}

declare spir_func ptr @_Z29async_work_group_strided_copyPU3AS1Dv16_sPU3AS3KS_jj9ocl_event(ptr addrspace(1), ptr addrspace(3), i32, i32, ptr)
...
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr [16 x i16], ptr addrspace(1) %dst, i32 [[dstiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr [16 x i16], ptr addrspace(3) %src, i32 [[phiiterator]]

; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]

; CHECK: [[ld:%[a-zA-Z0-9_.]+]] = load [16 x i16], ptr addrspace(3) [[srci]]
; CHECK: store [16 x i16] [[ld]], ptr addrspace(1) [[dsti]]
