...
define dso_local spir_func %opencl.event_t* @foo(<8 x i32> addrspace(1)* %dst, <8 x i32> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv8_iPU3AS3KS_j9ocl_event(<8 x i32> addrspace(1)* %dst, <8 x i32> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv8_iPU3AS3KS_j9ocl_event(<8 x i32> addrspace(1)*, <8 x i32> addrspace(3)*, i32, %opencl.event_t*)
...
; CHECK: [[ld:%[a-zA-Z0-9_.]+]] = load [8 x i32], [8 x i32] addrspace(3)* [[srci]]
; CHECK: store [8 x i32] [[ld]], [8 x i32] addrspace(1)* [[dsti]]
