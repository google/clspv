...
define dso_local spir_func %opencl.event_t* @foo(<3 x i32> addrspace(1)* %dst, <3 x i32> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv3_iPU3AS3KS_j9ocl_event(<3 x i32> addrspace(1)* %dst, <3 x i32> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv3_iPU3AS3KS_j9ocl_event(<3 x i32> addrspace(1)*, <3 x i32> addrspace(3)*, i32, %opencl.event_t*)
...
; CHECK: [[loop]]:
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr <3 x i32>, <3 x i32> addrspace(1)* %dst, i32 [[phiiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr <3 x i32>, <3 x i32> addrspace(3)* %src, i32 [[phiiterator]]
; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]
; CHECK: call void @_Z8spirv.op.63.PU3AS1Dv3_jPU3AS3Dv3_j(i32 63, <3 x i32> addrspace(1)* [[dsti]], <3 x i32> addrspace(3)* [[srci]])
; CHECK: br label %[[cmp]]
