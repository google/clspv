...
define dso_local spir_func %opencl.event_t* @foo(<4 x i64> addrspace(1)* %dst, <4 x i64> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv4_lPU3AS3KS_j9ocl_event(<4 x i64> addrspace(1)* %dst, <4 x i64> addrspace(3)* %src, i32 %num_gentypes, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS1Dv4_lPU3AS3KS_j9ocl_event(<4 x i64> addrspace(1)*, <4 x i64> addrspace(3)*, i32, %opencl.event_t*)
...
; CHECK: [[loop]]:
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr <4 x i64>, <4 x i64> addrspace(1)* %dst, i32 [[phiiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr <4 x i64>, <4 x i64> addrspace(3)* %src, i32 [[phiiterator]]
; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]
; CHECK: call void @_Z8spirv.op.63.PU3AS1Dv4_mPU3AS3Dv4_m(i32 63, <4 x i64> addrspace(1)* [[dsti]], <4 x i64> addrspace(3)* [[srci]])
; CHECK: br label %[[cmp]]
