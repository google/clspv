...
define dso_local spir_func %opencl.event_t* @foo(<3 x i32> addrspace(3)* %dst, <3 x i32> addrspace(1)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS3Dv3_iPU3AS1KS_jj9ocl_event(<3 x i32> addrspace(3)* %dst, <3 x i32> addrspace(1)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS3Dv3_iPU3AS1KS_jj9ocl_event(<3 x i32> addrspace(3)*, <3 x i32> addrspace(1)*, i32, i32, %opencl.event_t*)
...
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr <3 x i32>, <3 x i32> addrspace(1)* %src, i32 [[srciterator]]
