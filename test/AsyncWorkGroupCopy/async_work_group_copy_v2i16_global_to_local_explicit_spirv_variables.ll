...
define dso_local spir_func %opencl.event_t* @foo(<2 x i16> addrspace(3)* %dst, <2 x i16> addrspace(1)* %src, i32 %num_gentypes, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS3Dv2_sPU3AS1KS_j9ocl_event(<2 x i16> addrspace(3)* %dst, <2 x i16> addrspace(1)* %src, i32 %num_gentypes, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS3Dv2_sPU3AS1KS_j9ocl_event(<2 x i16> addrspace(3)*, <2 x i16> addrspace(1)*, i32, %opencl.event_t*)
...
; CHECK: [[loop]]:
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr <2 x i16>, <2 x i16> addrspace(3)* %dst, i32 [[phiiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr <2 x i16>, <2 x i16> addrspace(1)* %src, i32 [[phiiterator]]
; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]
; CHECK: call void @_Z8spirv.op.63.PU3AS3Dv2_tPU3AS1Dv2_t(i32 63, <2 x i16> addrspace(3)* [[dsti]], <2 x i16> addrspace(1)* [[srci]])
; CHECK: br label %[[cmp]]
