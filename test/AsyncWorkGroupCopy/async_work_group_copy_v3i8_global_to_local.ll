...
define dso_local spir_func %opencl.event_t* @foo(<3 x i8> addrspace(3)* %dst, <3 x i8> addrspace(1)* %src, i32 %num_gentypes, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS3Dv3_cPU3AS1KS_j9ocl_event(<3 x i8> addrspace(3)* %dst, <3 x i8> addrspace(1)* %src, i32 %num_gentypes, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z21async_work_group_copyPU3AS3Dv3_cPU3AS1KS_j9ocl_event(<3 x i8> addrspace(3)*, <3 x i8> addrspace(1)*, i32, %opencl.event_t*)
...
; CHECK: [[loop]]:
; CHECK: [[dsti:%[a-zA-Z0-9_.]+]] = getelementptr <3 x i8>, <3 x i8> addrspace(3)* %dst, i32 [[phiiterator]]
; CHECK: [[srci:%[a-zA-Z0-9_.]+]] = getelementptr <3 x i8>, <3 x i8> addrspace(1)* %src, i32 [[phiiterator]]
; CHECK: [[nextiterator]] = add i32 [[phiiterator]], [[incr]]
; CHECK: call void @_Z8spirv.op.63.PU3AS3Dv3_hPU3AS1Dv3_h(i32 63, <3 x i8> addrspace(3)* [[dsti]], <3 x i8> addrspace(1)* [[srci]])
; CHECK: br label %[[cmp]]
