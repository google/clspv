...
define dso_local spir_func %opencl.event_t* @foo(<8 x i64> addrspace(3)* %dst, <8 x i64> addrspace(1)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event) {
entry:
  %call = call spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS3Dv8_lPU3AS1KS_jj9ocl_event(<8 x i64> addrspace(3)* %dst, <8 x i64> addrspace(1)* %src, i32 %num_gentypes, i32 %stride, %opencl.event_t* %event)
  ret %opencl.event_t* %call
}

declare spir_func %opencl.event_t* @_Z29async_work_group_strided_copyPU3AS3Dv8_lPU3AS1KS_jj9ocl_event(<8 x i64> addrspace(3)*, <8 x i64> addrspace(1)*, i32, i32, %opencl.event_t*)
...
; CHECK: [[ld:%[a-zA-Z0-9_.]+]] = load [8 x i64], [8 x i64] addrspace(1)* [[srci]]
; CHECK: store [8 x i64] [[ld]], [8 x i64] addrspace(3)* [[dsti]]
