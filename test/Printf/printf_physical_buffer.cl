// RUN: clspv -arch=spirv64 %s -o %t.spv -enable-printf -physical-storage-buffers
...
// CHECK-DAG: %[[runtimearr:[0-9a-zA-Z_]+]] = OpTypeRuntimeArray %[[uint]]
// CHECK-DAG: %[[struct:[0-9a-zA-Z_]+]] = OpTypeStruct %[[runtimearr]]
// CHECK-DAG: %[[printf_buffer_ty_:[0-9a-zA-Z_]+]] = OpTypePointer PhysicalStorageBuffer %[[struct]]
...
// CHECK: %[[printf_buffer:[0-9a-zA-Z_]+]] = OpConvertUToPtr %[[printf_buffer_ty_]] %[[printf_buffer_pc]]
// CHECK: %[[printf_buffer_gep:[0-9a-zA-Z_]+]] = OpAccessChain %[[printf_buffer_ty]] %[[printf_buffer]] %[[zero]] %[[zero]]
// CHECK: %[[printf_offset:[0-9a-zA-Z_]+]] = OpAtomicIAdd %[[uint]] %[[printf_buffer_gep]] %{{[0-9a-zA-Z_]+}} %{{[0-9a-zA-Z_]+}} %[[one]]
