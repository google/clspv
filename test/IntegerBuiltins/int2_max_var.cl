...
// CHECK: OpDecorate %[[UINT_DYNAMIC_VECTOR_ARRAY_TYPE_ID:[a-zA-Z0-9_]*]] ArrayStride 8
// CHECK: OpMemberDecorate %[[UINT_ARG_VECTOR_STRUCT_TYPE_ID:[a-zA-Z0-9_]*]] 0 Offset 0
// CHECK: OpDecorate %[[UINT_ARG_VECTOR_STRUCT_TYPE_ID]] BufferBlock
...
// CHECK: %[[VECTOR_SHUFFLE_ID:[a-zA-Z0-9_]*]] = OpVectorShuffle %[[UINT_VECTOR_TYPE_ID]] %[[COMPOSITE_INSERT_ID]] %[[CONSTANT_UNDEF_ID]] 0 0
...
void kernel __attribute__((reqd_work_group_size(1, 1, 1))) foo(global int2* a, global int2* b, global int* c)
{
  *a = max(*b, *c);
}
