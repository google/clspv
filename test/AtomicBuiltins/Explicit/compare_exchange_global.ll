...
define <3 x i1> @strong(ptr addrspace(1) %atomic, ptr addrspace(1) %expected) {
entry:
  %atomic_cast = addrspacecast ptr addrspace(1) %atomic to ptr addrspace(4)
  %expected_cast = addrspacecast ptr addrspace(1) %expected to ptr addrspace(4)
  ...
  %call2 = call spir_func zeroext i1 @_Z39atomic_compare_exchange_strong_explicitPU3AS4VU7_AtomiciPU3AS4ii12memory_orderS4_12memory_scope(ptr addrspace(4) %atomic_cast, ptr addrspace(4) %expected_cast, i32 997, i32 2, i32 0, i32 1)
...
define <3 x i1> @weak(ptr addrspace(1) %atomic, ptr addrspace(1) %expected) {
entry:
  %atomic_cast = addrspacecast ptr addrspace(1) %atomic to ptr addrspace(4)
  %expected_cast = addrspacecast ptr addrspace(1) %expected to ptr addrspace(4)
  ...
  %call2 = call spir_func zeroext i1 @_Z37atomic_compare_exchange_weak_explicitPU3AS4VU7_AtomiciPU3AS4ii12memory_orderS4_12memory_scope(ptr addrspace(4) %atomic_cast, ptr addrspace(4) %expected_cast, i32 997, i32 2, i32 0, i32 1)
...
; CHECK: [[cmp_xchg:%[a-zA-Z0-9_.]+]] = call i32 @_Z8spirv.op.230.{{.*}}(i32 230, ptr addrspace(1) %atomic, i32 1, i32 72, i32 66, i32 999, i32 [[load_expected]])
...
; CHECK: [[then]]:
; CHECK: store i32 [[cmp_xchg]], ptr addrspace(1) %expected
; CHECK: br label %[[fi]]
; CHECK: [[fi]]:
; CHECK: [[load_expected:%[a-zA-Z0-9_.]+]] = load i32, ptr addrspace(1) %expected
