...
; CHECK: [[gep:%[^ ]+]] = getelementptr half, half addrspace(1)* %a, i32 [[add4]]
; CHECK: [[ld4:%[^ ]+]] = load half, half addrspace(1)* [[gep]]
; CHECK: [[add5:%[^ ]+]] = add i32 [[add4]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr half, half addrspace(1)* %a, i32 [[add5]]
; CHECK: [[ld5:%[^ ]+]] = load half, half addrspace(1)* [[gep]]
; CHECK: [[add6:%[^ ]+]] = add i32 [[add5]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr half, half addrspace(1)* %a, i32 [[add6]]
; CHECK: [[ld6:%[^ ]+]] = load half, half addrspace(1)* [[gep]]
; CHECK: [[add7:%[^ ]+]] = add i32 [[add6]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr half, half addrspace(1)* %a, i32 [[add7]]
; CHECK: [[ld7:%[^ ]+]] = load half, half addrspace(1)* [[gep]]
...
define spir_kernel void @foo(half addrspace(1)* %a, <3 x i32> addrspace(1)* %b, i32 %i) {
entry:
  %0 = bitcast half addrspace(1)* %a to <3 x i32> addrspace(1)*
