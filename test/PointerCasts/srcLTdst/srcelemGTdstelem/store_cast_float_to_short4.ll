...
; CHECK:  [[gep:%[^ ]+]] = getelementptr float, ptr addrspace(1) %1, i32 [[shl]]
; CHECK:  store float [[bitcast0]], ptr addrspace(1) [[gep]]
; CHECK:  [[add:%[^ ]+]] = add i32 [[shl]], 1
; CHECK:  [[gep:%[^ ]+]] = getelementptr float, ptr addrspace(1) %1, i32 [[add]]
; CHECK:  store float [[bitcast1]], ptr addrspace(1) [[gep]]

define spir_kernel void @foo(ptr addrspace(1) %a, ptr addrspace(1) %b, i32 %i) {
entry:
  %0 = load <4 x i16>, ptr addrspace(1) %b, align 8
  %1 = getelementptr float, ptr addrspace(1) %a, i32 0
  %arrayidx = getelementptr inbounds <4 x i16>, ptr addrspace(1) %1, i32 %i
  store <4 x i16> %0, ptr addrspace(1) %arrayidx, align 8
  ret void
}
