...
; CHECK: [[gep:%[a-zA-Z0-9_.]+]] = getelementptr float, ptr addrspace(1) %0, i32 [[add1]]
; CHECK: [[ld2:%[a-zA-Z0-9_.]+]] = load float, ptr addrspace(1) [[gep]]
; CHECK: [[add2:%[a-zA-Z0-9_.]+]] = add i32 [[add1]], 1
; CHECK: [[gep:%[a-zA-Z0-9_.]+]] = getelementptr float, ptr addrspace(1) %0, i32 [[add2]]
; CHECK: [[ld3:%[a-zA-Z0-9_.]+]] = load float, ptr addrspace(1) [[gep]]
; CHECK: [[in0:%[a-zA-Z0-9_.]+]] = insertelement <4 x float> poison, float [[ld0]], i32 0
...
entry:
  ...
  %arrayidx = getelementptr inbounds <4 x i32>, ptr addrspace(1) %0, i32 %i
  %1 = load <4 x i32>, ptr addrspace(1) %arrayidx, align 8
  store <4 x i32> %1, ptr addrspace(1) %b, align 8
  ret void
}
