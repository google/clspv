...
; CHECK: [[gep:%[^ ]+]] = getelementptr i8, i8 addrspace(1)* %a, i32 [[shl]]
; CHECK: [[ld0:%[^ ]+]] = load i8, i8 addrspace(1)* [[gep]]
; CHECK: [[add:%[^ ]+]] = add i32 [[shl]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr i8, i8 addrspace(1)* %a, i32 [[add]]
; CHECK: [[ld1:%[^ ]+]] = load i8, i8 addrspace(1)* [[gep]]
; CHECK: [[add2:%[^ ]+]] = add i32 [[add]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr i8, i8 addrspace(1)* %a, i32 [[add2]]
; CHECK: [[ld2:%[^ ]+]] = load i8, i8 addrspace(1)* [[gep]]
; CHECK: [[add3:%[^ ]+]] = add i32 [[add2]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr i8, i8 addrspace(1)* %a, i32 [[add3]]
; CHECK: [[ld3:%[^ ]+]] = load i8, i8 addrspace(1)* [[gep]]
...
define spir_kernel void @foo(i8 addrspace(1)* %a, <2 x half> addrspace(1)* %b, i32 %i) {
entry:
  %0 = bitcast i8 addrspace(1)* %a to <2 x half> addrspace(1)*
