...
; CHECK: [[ld0:%[^ ]+]] = load i16, i16 addrspace(1)* [[gep]]
; CHECK: [[add:%[^ ]+]] = add i32 [[shl]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr i16, i16 addrspace(1)* %a, i32 [[add]]
; CHECK: [[ld1:%[^ ]+]] = load i16, i16 addrspace(1)* [[gep]]
...
define spir_kernel void @foo(i16 addrspace(1)* %a, <2 x half> addrspace(1)* %b, i32 %i) {
entry:
  %0 = bitcast i16 addrspace(1)* %a to <2 x half> addrspace(1)*
