...
; CHECK: [[gep:%[^ ]+]] = getelementptr [8 x half], ptr addrspace(1) %0, i32 [[shr]], i32 [[and]]
; CHECK: [[ld0:%[^ ]+]] = load half, ptr addrspace(1) [[gep]]
; CHECK: [[add:%[^ ]+]] = add i32 [[and]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr [8 x half], ptr addrspace(1) %0, i32 [[shr]], i32 [[add]]
; CHECK: [[ld1:%[^ ]+]] = load half, ptr addrspace(1) [[gep]]
; CHECK: [[add2:%[^ ]+]] = add i32 [[add]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr [8 x half], ptr addrspace(1) %0, i32 [[shr]], i32 [[add2]]
; CHECK: [[ld2:%[^ ]+]] = load half, ptr addrspace(1) [[gep]]
; CHECK: [[add3:%[^ ]+]] = add i32 [[add2]], 1
; CHECK: [[gep:%[^ ]+]] = getelementptr [8 x half], ptr addrspace(1) %0, i32 [[shr]], i32 [[add3]]
