...
; CHECK: [[gep:%[a-zA-Z0-9_.]+]] = getelementptr <2 x float>, <2 x float> addrspace(1)* %b, i32 [[shr]]
; CHECK: [[ld0:%[a-zA-Z0-9_.]+]] = load <2 x float>, <2 x float> addrspace(1)* [[gep]]
; CHECK: [[and:%[a-zA-Z0-9_.]+]] = and i32 %i, 1
; CHECK: [[ex0:%[a-zA-Z0-9_.]+]] = extractelement <2 x float> [[ld0]], i32 [[and]]
; CHECK: [[cast:%[a-zA-Z0-9_.]+]] = bitcast float [[ex0]] to i32
define spir_kernel void @foo(i32 addrspace(1)* %a, <2 x float> addrspace(1)* %b, i32 %i) {
entry:
  %0 = bitcast <2 x float> addrspace(1)* %b to i32 addrspace(1)*
  %arrayidx = getelementptr inbounds i32, i32 addrspace(1)* %0, i32 %i
