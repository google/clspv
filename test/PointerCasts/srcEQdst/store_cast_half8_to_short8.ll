...
; CHECK: [[ld:%[^ ]+]] = load [8 x i16], [8 x i16] addrspace(1)* %b
...
; CHECK: [[insert0:%[^ ]+]] = insertvalue [8 x half] undef, half [[bitcast0]], 0
...
; CHECK: store [8 x half] [[insert7]], [8 x half] addrspace(1)* [[gep]]

define spir_kernel void @foo([8 x half] addrspace(1)* %a, [8 x i16] addrspace(1)* %b, i32 %i) {
entry:
  %0 = load [8 x i16], [8 x i16] addrspace(1)* %b, align 8
  %1 = bitcast [8 x half] addrspace(1)* %a to [8 x i16] addrspace(1)*
  %arrayidx = getelementptr inbounds [8 x i16], [8 x i16] addrspace(1)* %1, i32 %i
  store [8 x i16] %0, [8 x i16] addrspace(1)* %arrayidx, align 8
  ret void
}
