...
; CHECK: [[ld:%[^ ]+]] = load [8 x i8], ptr addrspace(1) %b
...
; CHECK: [[insert0:%[^ ]+]] = insertelement <4 x i8> poison, i8 [[extract0]], i32 0
...
; CHECK: [[insert4:%[^ ]+]] = insertelement <4 x i8> poison, i8 [[extract4]], i32 0
...
; CHECK: [[gep:%[^ ]+]] = getelementptr <4 x half>, ptr addrspace(1) %1, i32 %i
; CHECK: store <4 x half> [[shuffle]], ptr addrspace(1) [[gep]]

define spir_kernel void @foo(ptr addrspace(1) %a, ptr addrspace(1) %b, i32 %i) {
entry:
  %0 = load [8 x i8], [8 x i8] addrspace(1)* %b, align 8
  %1 = getelementptr <4 x half>, ptr addrspace(1) %a, i32 0
  %arrayidx = getelementptr inbounds [8 x i8], ptr addrspace(1) %1, i32 %i
  store [8 x i8] %0, ptr addrspace(1) %arrayidx, align 8
  ret void
}
