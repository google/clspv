...
define void @foo(ptr addrspace(1) %in, <4 x double> %data, i32 %offset) {
entry:
  call spir_func void @_Z7vstore4Dv4_djPU3AS1d(<4 x double> %data, i32 %offset, ptr addrspace(1) %in)
  ret void
}

declare void @_Z7vstore4Dv4_djPU3AS1d(<4 x double>, i32, ptr addrspace(1))

; CHECK: [[mul:%[a-zA-Z0-9_.]+]] = mul i32 %offset, 4
; CHECK: [[add:%[a-zA-Z0-9_.]+]] = add i32 [[mul]], 0
; CHECK: [[gep:%[a-zA-Z0-9_.]+]] = getelementptr double, ptr addrspace(1) %in, i32 [[add]]
; CHECK: [[ex:%[a-zA-Z0-9_.]+]] = extractelement <4 x double> %data, i64 0
; CHECK: store double [[ex]], ptr addrspace(1) [[gep]]
