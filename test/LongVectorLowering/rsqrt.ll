...
; CHECK-COUNT-24: call spir_func float @_Z5rsqrtf(float
...
define dso_local spir_kernel void @rsqrtLongVecTest(ptr addrspace(1) align 32 %inout8, ptr addrspace(1) align 64 %inout16) #0 !kernel_arg_addr_space !8 !kernel_arg_access_qual !9 !kernel_arg_type !10 !kernel_arg_base_type !11 !kernel_arg_type_qual !12 !kernel_arg_name !13 !clspv.pod_args_impl !14 {
entry:
  ...
  %call = call spir_func <8 x float> @_Z5rsqrtDv8_f(<8 x float> %0) #2
  ...
  %call3 = call spir_func <16 x float> @_Z5rsqrtDv16_f(<16 x float> %3) #2
...
declare !kernel_arg_name !15 spir_func <8 x float> @_Z5rsqrtDv8_f(<8 x float>) #1

; Function Attrs: convergent nounwind willreturn memory(none)
declare !kernel_arg_name !15 spir_func <16 x float> @_Z5rsqrtDv16_f(<16 x float>) #1
