...
entry:
  ...
  %call = call spir_func <8 x float> @_Z4stepDv8_fS_(<8 x float> %0, <8 x float> %1)
  store <8 x float> %call, <8 x float> addrspace(1)* %dst, align 32
  ret void
}

declare spir_func <8 x float> @_Z4stepDv8_fS_(<8 x float>, <8 x float>)

; CHECK-LABEL: @test
; CHECK-NOT: call spir_func <8 x float> @_Z4stepDv8_fS_
; CHECK: {{.*}} = call spir_func float @_Z4stepff(float {{.*}}, float {{.*}})
; CHECK: {{.*}} = call spir_func float @_Z4stepff(float {{.*}}, float {{.*}})
; CHECK: {{.*}} = call spir_func float @_Z4stepff(float {{.*}}, float {{.*}})
; CHECK: {{.*}} = call spir_func float @_Z4stepff(float {{.*}}, float {{.*}})
; CHECK: {{.*}} = call spir_func float @_Z4stepff(float {{.*}}, float {{.*}})
