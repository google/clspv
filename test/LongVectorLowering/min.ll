...
entry:
  ...
  %call = call spir_func <8 x i32> @_Z3minDv8_iS_(<8 x i32> %0, <8 x i32> %1)
  store <8 x i32> %call, <8 x i32> addrspace(1)* %dst, align 32
  ret void
}

declare spir_func <8 x i32> @_Z3minDv8_iS_(<8 x i32>, <8 x i32>)

; CHECK-LABEL: @test
; CHECK-NOT: call spir_func <8 x i32> @_Z3minDv8_iS_
; CHECK: {{.*}} = call spir_func i32 @_Z3minii(i32 {{.*}}, i32 {{.*}})
; CHECK: {{.*}} = call spir_func i32 @_Z3minii(i32 {{.*}}, i32 {{.*}})
; CHECK: {{.*}} = call spir_func i32 @_Z3minii(i32 {{.*}}, i32 {{.*}})
; CHECK: {{.*}} = call spir_func i32 @_Z3minii(i32 {{.*}}, i32 {{.*}})
; CHECK: {{.*}} = call spir_func i32 @_Z3minii(i32 {{.*}}, i32 {{.*}})
