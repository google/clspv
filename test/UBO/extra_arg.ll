...
; CHECK: define {{.*}} @k1
; CHECK: [[val:%[a-zA-Z0-9_.]+]] = extractelement <4 x i32> {{.*}}, i64 0
; CHECK: call {{.*}} [[bar:@bar[a-zA-Z0-9_.]+]](i32 [[val]])
; CHECK: define {{.*}} [[bar]](i32 [[p:%[a-zA-Z0-9_.]+]])
; CHECK: [[res:%[a-zA-Z0-9_.]+]] = call ptr addrspace(2) @_Z14clspv.resource.1
; CHECK: [[gep:%[a-zA-Z0-9_.]+]] = getelementptr { [4096 x <4 x i32>] }, ptr addrspace(2) [[res]], i32 0, i32 0, i32 [[p]]
...
define dso_local spir_func <4 x i32> @bar(ptr addrspace(2) nocapture readonly %in) {
entry:
  %0 = load <4 x i32>, ptr addrspace(2) %in, align 16
  ret <4 x i32> %0
}
...
entry:
  ...
  %call = tail call spir_func <4 x i32> @bar(ptr addrspace(2) %6)
